...
const api = buildAPI(['algos', 'files', 'net', 'strings', 'fns', 'datastructures', 'numbers']);
...
const inputSetValue = value => {
  ...
  if (controlInput.inputType === 'command'){ // COMMAND
    //console.log("Типикал команд")
  }
...
const commandLoop = () => {
  input('command', '.', (err, line) => { // PASSWORD
    exec(line);
    commandLoop();
  });
};
...
*/
const help = [
  '', 'Commands: algos, files, net, strings, fns, datastructures, numbers'
];
let data = [];
const exec = async line => {
  ...
  switch(cmd){
    case "files":
      executionData = (args) => "files";
      console.log("- = files = -");
      break;
    case "algo":
      executionData = (args) => "algo";
      console.log("- = algo = -");
      break;
    case "arrays":
      executionData = (args) => "arrays";
      console.log("- = arrays = -");
      break;
    case "net":
      executionData = (args) => "net";
      console.log("- = net = -");
      break;
    case "strings":
      executionData = (args) => "strings";
      console.log("- = strings = -");
      break;
}
  */
  const data = await api[cmd](args);
  //console.dir(executionData)
  print(data);
  commandLoop();
};
